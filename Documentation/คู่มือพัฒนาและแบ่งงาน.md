...
| `evaluateHand` | คืนประเภทมือ ค่าเปรียบเทียบ และ kickers ตามลำดับศักดิ์ไพ่ของกติกา Indian Poker (เช่น Trail/Trio, Pure Sequence, Sequence, Color, Pair, High Card) ให้ตรงตาม Test |
...
พัฒนา `receiveCards`, `placeBet`, `call`, `raiseTo`, `fold`, `showCards`, `seeCards`, `addChips` และ `resetForNewRound` ตามข้อกำหนดและ Test ที่มี พร้อมเพิ่ม Test สำหรับพฤติกรรมที่ยังไม่ครอบคลุม (เช่น `seeCards()`)
...
- `call` และ `raiseTo` หักชิปตามส่วนต่างจนถึงยอดเป้าหมาย โดย `Player` มีหน้าที่หักชิปตามยอดเป้าหมายที่ระบุมาเท่านั้น ส่วนการคำนวณยอดเดิมพันตามกติกาความต่างของ Blind/Seen ให้เป็นหน้าที่รับผิดชอบของ `GameState`
- ปฏิเสธการกระทำที่ไม่ถูกต้องโดยไม่เปลี่ยนยอดเงินหรือสถานะไปบางส่วน
- `resetForNewRound` ล้างไพ่และยอดเดิมพัน เปลี่ยนสถานะกลับสำหรับรอบใหม่ และรักษาชิปที่เหลือ
...
Player จัดการข้อมูลรายคน ส่วน GameState จัดการกองกลาง เทิร์น และการตัดสินทั้งโต๊ะ ต้องตกลงให้ชัดว่าจุดใดหักชิป เพื่อไม่หักซ้ำซ้อน
...
พัฒนาลอจิกการเริ่มรอบ เก็บเงินกองกลางเริ่มต้น (Boot) แจกไพ่ ประมวลผลการกระทำ (Action) การสลับเทิร์น และเงื่อนไขการจบรอบให้ครอบคลุมกติกาของ Indian Poker

**ข้อกำหนดที่ตกลงแล้ว:**
- **การ Show ปกติ:** จะกระทำได้ก็ต่อเมื่อเหลือผู้เล่นที่ยังไม่หมอบ (Active) เพียง 2 คนในโต๊ะเท่านั้น
- **กรณีเหลือผู้เล่นคนเดียว:** หากผู้เล่นคนอื่นหมอบ (Fold) จนเหลือผู้เล่นในเกมเพียง 1 คน ผู้เล่นคนนั้นจะชนะและรับกองกลางทันทีโดยไม่จำเป็นต้องเปิดไพ่ให้ใครเห็น
- **ตัวอย่างการคำนวณยอดเดิมพัน (Blind/Seen):**
  - สมมติ `currentHighestBet = 100` (ยอดเป้าหมายอ้างอิงจากสถานะ Seen)
  - ผู้เล่น A (Blind) ต้องการตาม (Call): หักชิป `100 / 2 = 50` ชิป (กองกลางเพิ่ม 50)
  - ผู้เล่น B (Seen) ต้องการตาม (Call): หักชิป `100` ชิป (กองกลางเพิ่ม 100)
  - ผู้เล่น C (เปลี่ยนจาก Blind เป็น Seen กลางรอบ) แล้วต้องการตาม (Call): หักชิป `100` ชิป
- ใช้ `Core` สำหรับการประมวลผลคำนวณผลแพ้ชนะ และใช้ `Player` สำหรับการปรับปรุงสถานะและข้อมูลรายบุคคล

**งานที่ต้องตกลงร่วมกันก่อนเชื่อมระบบ:**
- **ระบบ Sideshow:** การขอเทียบไพ่กับผู้เล่นคนก่อนหน้า เมธอด `executeSideshow()` เป็นเพียงขั้นตอน *หลัง* จากการตรวจสิทธิ์และยอมรับคำขอแล้วเท่านั้น (Test 4.8 อ้างอิงตัวละครที่ยังเป็น Blind ทั้งคู่ ซึ่งขัดกับกติกาใหม่ ต้องไปปรับปรุง Fixture ต่อไป) นอกจากนี้ ทีม GameState, Network และ Client ต้องตกลงร่วมกันเรื่อง: วิธีระบุผู้ขอ/ผู้รับคำขอ, รูปแบบ Event สำหรับการขอยอมรับ/ปฏิเสธ/หมดเวลา, การเดินเทิร์นระหว่างรอคำตอบ, และผลลัพธ์กรณีไพ่เสมอกัน
- **ระบบ Force Show (บังคับเปิดไพ่):** เกิดขึ้นเมื่อยอดเงินกองกลางถึงเพดานสูงสุด (Pot Limit) แม้ยังมีผู้เล่นมากกว่า 2 คน ระบบจะต้องบังคับจบรอบและเปิดไพ่เทียบกัน (ปัจจุบัน Source มี `canForceShow()` แต่ Test 4.9 ตรวจแค่เพดานเงิน ยังไม่ได้ตรวจการบังคับเปิดไพ่และการจ่ายเงินอย่างครบวงจร)
...
**งานที่ต้องตกลงร่วมกันก่อนเชื่อมระบบ:**
- **การส่งไพ่ในสถานะ Blind:** สำหรับ `myCards` ต้องตกลงกันว่าเมื่อผู้เล่นอยู่ในสถานะ Blind ทาง Server จะส่ง `myCards: []` ให้ และจะส่งไพ่จริงให้ก็ต่อเมื่อคำสั่ง `SEEN` สำเร็จ (หากเลือกแนวทางนี้ ต้องไปปรับ Fixture และ Test ปัจจุบันที่คาดหวังไพ่ทั้งที่ยังเป็น Blind เพื่อหลีกเลี่ยงการส่งไพ่ไปให้ Client ซ่อนไว้เอง)
