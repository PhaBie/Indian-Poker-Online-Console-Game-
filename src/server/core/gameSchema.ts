...
export const shuffleDeckInputSchema = z
  .array(cardSchema)
  .refine((cardList) => !hasDuplicateCards(cardList), {
...
export const dealCardsInputSchema = z
  .object({
  ...
  .refine(
    ...
    {
      message:
        'Insufficient cards in the deck to deal the required amount to all players.',
    },
  );

// 3. กำหนดเงื่อนไขไพ่ในมือสำหรับนำไปประเมินผล: ต้องมีไพ่จำนวน 3 ใบถ้วน และไพ่ทั้ง 3 ใบต้องไม่ซ้ำกัน
export const evaluateHandInputSchema = z
  .array(cardSchema)
...
export const compareHandsInputSchema = z
  .object({
...
export const playerHandSchema = z
  .object({
    id: z.string().min(1),
...
export const getWinnersInputSchema = z
  .array(playerHandSchema)
  .refine(
    (playerList) => {
...
export const calculateSplitPotInputSchema = z
  .object({
  ...
  .strict()
  .superRefine((splitData, validationContext) => {
    const hasNoWinners = splitData.winnerIds.length === 0;
    if (splitData.pot > 0 && hasNoWinners) {
      validationContext.addIssue({
        code: 'custom',
        message:
          'Invalid payout configuration: the pot is greater than 0, but no winners were provided.',
      });
    }
  });
