...
} from '../../../01-Source-code/server/core/gameSchema';
import type { Card } from '../../../01-Source-code/shared/types';

describe('การทดสอบ Validation ของ gameSchema', () => {
  describe('1. ข้อมูลพื้นฐาน (Base Types)', () => {
    test('1.1 cardSchema - ข้อมูลไพ่ถูกต้อง', () => {
      expect(cardSchema.parse({ suit: 'SPADES', rank: 14 })).toEqual({
        suit: 'SPADES',
        rank: 14,
      });
    });
    ...
    test('1.4.1 cardSchema - แต้มไพ่ไม่รับค่าที่ผิดประเภท (ทศนิยม, String, NaN, Infinity)', () => {
      ...
      expect(cardSchema.safeParse({ suit: 'SPADES', rank: Infinity }).success).toBe(
        false,
      );
    });
    ...
    test('1.6 positiveSafeIntSchema - ขอบเขตจำนวนเต็มบวก (ไม่รวมศูนย์)', () => {
      ...
      expect(positiveSafeIntSchema.parse(Number.MAX_SAFE_INTEGER)).toBe(
      ...
      expect(positiveSafeIntSchema.safeParse(Number.MAX_SAFE_INTEGER + 1).success).toBe(
        false,
      );
    ...
    test('1.7 nonNegativeSafeIntSchema - ขอบเขตจำนวนเต็มบวกหรือศูนย์ (รวมศูนย์)', () => {
      ...
      expect(nonNegativeSafeIntSchema.parse(Number.MAX_SAFE_INTEGER)).toBe(
        Number.MAX_SAFE_INTEGER,
      );
      ...
      expect(
        nonNegativeSafeIntSchema.safeParse(Number.MAX_SAFE_INTEGER + 1).success,
      ).toBe(false);
  ...
  describe('4. evaluateHandInputSchema (ไพ่ในมือสำหรับการประเมิน)', () => {
    ...
    test('4.2 ข้อมูลไพ่ไม่ถูกต้อง (จำนวนไพ่ไม่เท่ากับ 3 ใบ)', () => {
      expect(
        evaluateHandInputSchema.safeParse([{ suit: 'HEARTS', rank: 2 }]).success,
      ).toBe(false);
      expect(
        evaluateHandInputSchema.safeParse([
          { suit: 'HEARTS', rank: 2 },
          { suit: 'SPADES', rank: 3 },
          { suit: 'CLUBS', rank: 4 },
  ...
  describe('5. compareHandsInputSchema (มือไพ่สำหรับการเปรียบเทียบ)', () => {
    test('5.1 ข้อมูลถูกต้อง (มือไพ่สองมืออิสระต่อกันและไม่ผิดกฎ)', () => {
      ...
      expect(compareHandsInputSchema.parse({ firstHand, secondHand })).toEqual({
        firstHand,
        secondHand,
      });
    });
    ...
    test('5.2 ข้อมูลถูกต้อง (มือไพ่สองมือมีไพ่เหมือนกันทุกประการ อนุญาตให้ทำได้เนื่องจากประเมินแยกกัน)', () => {
      ...
      expect(
        compareHandsInputSchema.parse({ firstHand: hand, secondHand: hand }),
      ).toEqual({
        firstHand: hand,
        secondHand: hand,
      });
  ...
  describe('7. calculateSplitPotInputSchema (ข้อมูลสำหรับการแบ่งกองกลาง)', () => {
    test('7.1 ข้อมูลถูกต้อง (เงินกองกลางเป็นบวกและมีผู้ชนะหลายคน)', () => {
      expect(
        calculateSplitPotInputSchema.parse({ pot: 1000, winnerIds: ['p1', 'p2'] }),
      ).toEqual({
        pot: 1000,
        winnerIds: ['p1', 'p2'],
      });
    ...
    test('7.2 ข้อมูลถูกต้อง (เงินกองกลางเป็น 0 และไม่มีผู้ชนะ)', () => {
      expect(calculateSplitPotInputSchema.parse({ pot: 0, winnerIds: [] })).toEqual({
        pot: 0,
        winnerIds: [],
      });
    });
    ...
    test('7.4 ข้อมูลไม่ถูกต้อง (เงินกองกลางติดลบ)', () => {
      expect(
        calculateSplitPotInputSchema.safeParse({ pot: -100, winnerIds: ['p1'] }).success,
      ).toBe(false);
    });

    test('7.5 ข้อมูลไม่ถูกต้อง (เงินกองกลางเป็นทศนิยม)', () => {
      expect(
        calculateSplitPotInputSchema.safeParse({ pot: 100.5, winnerIds: ['p1'] }).success,
      ).toBe(false);
    });

    test('7.6 ข้อมูลไม่ถูกต้อง (พบ ID ผู้ชนะซ้ำซ้อน)', () => {
      expect(
        calculateSplitPotInputSchema.safeParse({ pot: 1000, winnerIds: ['p1', 'p1'] })
          .success,
      ).toBe(false);
    });
  });
