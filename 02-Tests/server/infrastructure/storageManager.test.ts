...
import { StorageManager } from '../../../01-Source-code/server/infrastructure/StorageManager';
import { Room } from '../../../01-Source-code/server/domain/models/Room';
...
describe('9. ระบบบันทึกและกู้คืนสถานะ (Storage Manager)', () => {
  ...
  test('[StorageManager.saveRoomState → loadRoomState] 9.1 บันทึกสถานะห้องลงไฟล์ → โหลดกลับแล้ว roomId, phase, pot, currentStake และไพ่ผู้เล่นคนแรกตรงตามที่บันทึก', () => {
    withTempDir((dir) => {
      ...
      const savedFirstPlayer = rawJson.gameState.activePlayers.find(
        (player: { id: string; privateCards?: unknown[] }) => player.id === 'id_p1',
      );
      ...
      const loadedFirstPlayer = loadedRoom?.gameState?.activePlayers.find(
        (player) => player.id === 'id_p1',
      );
